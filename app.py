...
st.title("📦 통합 마트 수주 자동 변환기")
st.markdown("Tesco, 이마트(트레이더스, 노브랜드), 롯데마트의 수주 데이터를 하나의 표준 양식으로 자동 변환합니다.")
...
# =====================================================================
with tab_tesco:
    st.subheader("🔴 Tesco 발주 데이터 자동 변환")
    ...
    file_tesco = st.file_uploader("📥 Tesco 주문서 업로드 (csv/xlsx)", type=['xlsx', 'xls', 'csv'], key="tesco")

    if file_tesco:
        try:
            with st.spinner("Tesco 데이터 통합 변환 중..."):
                all_rows = []
                ...
                st.success(f"✅ Tesco 처리 완료! ({len(df_final)}건)")
                st.dataframe(df_final.head(10))
                st.download_button("📥 통합 양식 다운로드 (Tesco)", 
...
# =====================================================================
with tab_emart:
    st.subheader("🟡 이마트 (이마트/TRD/노브랜드) 발주 데이터 자동 변환")
    ...
    if prod_df is None or prod_df.empty:
        st.warning("⚠️ 서버에 이마트 마스터 파일(깃허브 서식파일)이 없습니다.")
    else:
        file_emart = st.file_uploader("📥 이마트 일반 주문서(Raw Data) 업로드", type=['xlsx', 'xls', 'csv'], key="emart")
        
        if file_emart:
            try:
                with st.spinner("이마트 데이터 통합 변환 중..."):
                    ...
                    st.success(f"✅ 이마트 처리 완료! ({len(df_final)}건)")
                    st.dataframe(df_final.head(10))
                    st.download_button("📥 통합 양식 다운로드 (이마트)", 
                                       data=to_excel_unified(df_final), 
...
# =====================================================================
with tab_lotte:
    st.subheader("🟢 롯데마트 발주 데이터 자동 변환")
    ...
    file_lotte = st.file_uploader("📥 EDI Raw Data (xls/csv) 업로드", type=['xlsx', 'csv'], key="lotte")
    
    if file_lotte:
        try:
            with st.spinner("롯데마트 데이터 통합 변환 중..."):
                ...
                if not parsed_list:
                    st.warning("유효한 롯데마트 발주 내역이 없습니다.")
                else:
                    ...
                    if os.path.exists(LOTTE_TEMPLATE):
                        ...
                        st.warning("롯데마트 서식파일을 찾을 수 없어 EDI 원본 단가/품명으로 산출합니다.")
                    ...
                    st.success(f"✅ 롯데마트 처리 완료! ({len(df_final)}건)")
                    st.dataframe(df_final.head(10))
                    st.download_button("📥 통합 양식 다운로드 (롯데마트)", 
                                       data=to_excel_unified(df_final), 
